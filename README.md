...
The application meant to be deployed is the **Item catalog app**, previously developed for [Project 3](https://github.com/izzyandrade/catalog).

## Useful info

IP address: 18.219.247.85

Accessible SSH port: 2200.

Application URL: [http://ec2-18-219-247-85.us-east-2.compute.amazonaws.com/](http://ec2-18-219-247-85.us-east-2.compute.amazonaws.com/).
...
4. Now you are able to log into the remote VM through ssh with the following command: `$ ssh -i ~/.ssh/udacity_key.rsa grader@ 18.219.247.85`.
...
2. `$ sudo service ssh restart`.
3. Now you are able to log into the remote VM through ssh with the following command: `$ ssh -i ~/.ssh/udacity_key.rsa -p 2200 grader@ 18.219.247.85`.
...
3. Move inside that newly created folder: `$ cd /catalog` and clone the catalog repository from Github: `$ git clone https://github.com/izzyandrade/catalog.git catalog`.
...
**Notes**: the *.git* folder will be inaccessible from the web without any particular setting. The only directory that can be listed in the browser will be the static folder: [static assets](http://ec2-18-219-247-85.us-east-2.compute.amazonaws.com/static/).
...
```
<VirtualHost *:80>
    ServerName 18.219.247.85
    ServerAlias http://ec2-18-219-247-85.us-east-2.compute.amazonaws.com/
    ServerAdmin admin@18.219.247.85
...
1. To let users correctly log-in change the authorized URI to [http://ec2-18-219-247-85.us-east-2.compute.amazonaws.com/](http://ec2-18-219-247-85.us-east-2.compute.amazonaws.com/) on Google developer dashboard.

### 19 - Restart Apache to launch the app
1. `$ sudo service apache2 restart`.

#### Special thanks to [*iliketomatoes*](https://github.com/iliketomatoes) who wrote a really helpful README for this project.
